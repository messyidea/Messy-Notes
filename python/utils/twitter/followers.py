...
def push_to_file(ids):
    ...
    for i in range(0, len(json_lookups)):
        # if i == 5:
        #     print json_lookups[i]
		#fields
        friendcount = json_lookups[i]["friends_count"]
        if friendcount is None:
            friendcount = 0
        statuscount = json_lookups[i]["statuses_count"]
        if statuscount is None:
            statuscount = 0
        ...
        if followers_count > 3 and followers_count < 10:
            fd.write(str(followers_count) + "======" + str(friendcount) + "------"+ str(statuscount) + "         " +name.replace(',',' ') + ',' + screen_name + ',' + str(followers_count_category) + ',' + description.replace('\r\n',' ').replace('\r',' ').replace('\n',' ').replace(',',' ') + ',' + location.replace(',',' ') + ',' + lang)
            fd.write('\n')
    fd.close()
