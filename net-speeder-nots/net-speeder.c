...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
	...
	if(ip->ip_ttl != SPECIAL_TTL) {
		ip->ip_ttl = SPECIAL_TTL;   //初始化ttl?
		int len_written = libnet_adv_write_raw_ipv4(libnet_handler, (u_int8_t *)ip, ntohs(ip->ip_len)); //重发包
		if(len_written < 0) {
...
libnet_t* start_libnet(char *dev) {
	...
	libnet_t *libnet_handler = libnet_init(LIBNET_RAW4_ADV, dev, errbuf);   //初始化libnet函数库
...
int main(int argc, char **argv) {
	...
	if (argc == ARGC_NUM) {         //参数数量
		dev = argv[1];              //openvz架构的公网
		filter_rule = argv[2];      //规则，"ip" or "tcp" etc
		printf("Device: %s\n", dev);
	...
	if (pcap_lookupnet(dev, &net, &mask, errbuf) == -1) {   //查找可用的网卡
	...
	if (pcap_compile(handle, &fp, filter_rule, 0, net) == -1) {     //编译和设置过滤器
		printf("filter rule err:[%s][%s]\n", filter_rule, pcap_geterr(handle));
		return -1;
	}

	if (pcap_setfilter(handle, &fp) == -1) {        //指定一个过滤程序
